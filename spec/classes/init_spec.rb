  ...
  context 'Supported OS - ' do
    ['Debian', 'RedHat'].each do |osfamily|
      ...
      describe "#{osfamily} configuration via custom template" do
        ...
        it { should contain_file('memcached.conf').with_content(/This is a template used only for rspec tests/) }
      ...
      describe "#{osfamily} configuration via custom content" do
        ...
        it { should contain_file('memcached.conf').with_content(/my_content/) }
      end

      describe "#{osfamily} configuration via custom source file" do
        let(:params) { {
          :config_file_source => "puppet:///modules/memcached/spec.conf",
        } }
        let(:facts) {{
          :osfamily => osfamily,
        }}
        it { should contain_file('memcached.conf').with_source('puppet:///modules/memcached/spec.conf') }
      end
      ...
      describe "#{osfamily} service restart on config file change (default)" do
        ...
        it 'should automatically restart the service when files change' do
          should contain_file('memcached.conf').with_notify('Service[memcached]')
        end
      end
      ...
      describe "#{osfamily} service restart disabling on config file change" do
        ...
        it 'should automatically restart the service when files change' do
          should contain_file('memcached.conf').without_notify
        end
      end
